public class Dandy.DrawStalk {
	...
	public static Details gen_details(Params params) {
		Details details = Details() {
			leafs = new LeafDetails[params.head.leafs.count]
		};
		for (uint leaf_idx = 0; leaf_idx < params.head.leafs.count; ++leaf_idx) {
			double x0 =
				(leaf_idx / (params.head.leafs.count - 1) - 0.5)
				* (params.head.width - params.head.leafs.base_width);
			double y0 = 0;
			double x1 = x0 + 0.5 * Util.random_sym(params.head.leafs.max_offset);
			double y1 = y0
				+ 0.2 * params.head.leafs.len * Random.double_range(0.9, 1.1);
			double x3 = x0 + Util.random_sym(params.head.leafs.max_offset);
			double y3 = y0
				+ params.head.leafs.len * Random.double_range(0.7, 1.3);
			double x2 = x3 + 0.5 * Util.random_sym(params.head.leafs.max_offset);
			double y2 = y3
				- 0.2 * params.head.leafs.len * Random.double_range(0.9, 1.1);
			details.leafs[leaf_idx] = LeafDetails() {
	...
	public static Util.Bounds get_bounds(Params params) {
		...
		double padding = 8;
		double head_padding =
			0.5 * params.head.width
			+ 0.5 * params.head.height
			+ 0.5 * params.head.leafs.base_width
			+ params.head.leafs.max_offset;
		double x1 = -0.5 * params.stem.diam_start - head_padding - padding;
		double x2 = -x1;
		double y_offset = params.stem.len * slope_y * Util.min(
			1f / (4 * params.stem.droop),
			1 - params.stem.droop);
		double y1 = 0.5 * params.stem.diam_start + padding;
		double y2 = y_offset
			- 0.5 * params.head.width
			- 0.5 * params.head.height
			- padding;
	...
	public static Util.Point get_head_pos(StemParams stem) {
		return Util.Point() {
	...
	public static double get_head_angle(StemParams stem) {
		return Math.atan2(
			Math.cos(stem.angle),
	...
	public static void draw(
			...
			Params params,
			Details details) {
		Util.Point root_pos = DrawStalk.get_root_pos();
		Util.Point head_pos = DrawStalk.get_head_pos(params.stem);
		double head_angle = DrawStalk.get_head_angle(params.stem);

		ctx.save();
		ctx.translate(root_pos.x, root_pos.y);
		DrawStalk.draw_stem(ctx, params.stem);
		ctx.restore();

		ctx.save();
		ctx.translate(head_pos.x, head_pos.y);
		ctx.rotate(head_angle);
		DrawStalk.draw_head(ctx, params.head, details.leafs);
		ctx.restore();
	}
	...
	public static void draw_stem(
		...
		Util.Point head_pos = DrawStalk.get_head_pos(stem);
		Path center_path = Path.line_with_droop(
			slope_x, slope_y,
			stem.len,
			stem.droop);
		Path.OffsetParameterization offset = (t) => Util.lerp(
			0.5 * stem.diam_start,
			0.5 * stem.diam_end, t);
		Path stem_path =
			center_path.offset(offset)
			.append(
				center_path.offset((t) => -offset(t)).reverse());

		ctx.save();

		ctx.new_path();
		DrawUtil.path_to(ctx, stem_path, stem.seg_count);
		ctx.close_path();
		...
		Util.Point grad_start = Util.Point() {
			x = -grad_start_excess * slope_x,
			y = -grad_start_excess * slope_y
		};
		Util.Point grad_end = Util.Point() {
		...
		Util.Point light_grad_start = Util.Point() {
			x = light_grad_excess * slope_y,
			y = -light_grad_excess * slope_x
		};
		Util.Point light_grad_end = Util.Point() {
			x = -light_grad_excess * slope_y,
			y = light_grad_excess * slope_x
		};
	...
	public static void draw_head(
			Cairo.Context ctx,
			HeadParams head,
			LeafDetails[] leaf_details) {
		DrawStalk.draw_head_leafs(ctx, head, leaf_details);
		...
		DrawStalk.draw_head_stipples(ctx, head);
	}

	// Draws the leafs around the head, assuming head transformations.
	public static void draw_head_leafs(
			Cairo.Context ctx,
			HeadParams head,
			LeafDetails[] leaf_details) {
		ctx.save();
		foreach (LeafDetails leaf_detail in leaf_details) {
			Path center_path = Path.cubic_spline(
				leaf_detail.x0, leaf_detail.y0,
				leaf_detail.x1, leaf_detail.y1,
				leaf_detail.x2, leaf_detail.y2,
				leaf_detail.x3, leaf_detail.y2);
			Path.OffsetParameterization offset = (t) =>
				Util.lerp(0.5 * head.leafs.base_width, 0, t);
			Path stem_path =
				center_path.offset(offset)
				.append(
					center_path.offset((t) => -offset(t)).reverse());
			...
			ctx.save();
			ctx.new_path();
			DrawUtil.path_to(ctx, stem_path, head.leafs.seg_count);
			ctx.close_path();
			ctx.set_source_rgb(
				0.235 * leaf_detail.shade,
				0.353 * leaf_detail.shade,
				0.137 * leaf_detail.shade);
			ctx.fill();
			ctx.restore();
		}
		ctx.restore();
	...
	public static void draw_head_stipples(
			Cairo.Context ctx,
			HeadParams head) {
		uint ring_count =
		...
		for (uint ring_idx = 0; ring_idx < ring_count; ++ring_idx) {
			...
			double x1 = 0.5 * ellipse_diam_x
				* Math.sqrt(1 - Util.square(y1 / (0.5 * ellipse_diam_y)));
			if (ring_idx % 2 == 0) {
				x1 = -x1;
			}
			double x2 = 0;
